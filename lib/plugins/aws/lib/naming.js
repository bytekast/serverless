...
module.exports = {
  ...
  // MQ
  getMQEventLogicalId(functionName, queueName) {
    const normalizedFunctionName = this.getNormalizedFunctionName(functionName);
    // QueueName is trimmed to 158 chars to avoid going over 255 character limit
    const normalizedQueueName = this.normalizeNameToAlphaNumericOnly(queueName).slice(0, 158);
    return `${normalizedFunctionName}EventSourceMappingMQ${normalizedQueueName}`;
  },
