...
module.exports = {
  compileMethods() {
    ...
    const apiGatewayModels = this.serverless.service.provider.apiGateway.models || {};
    ...
    this.validated.events.forEach(event => {
      ...
      if (event.http.request && event.http.request.schema) {
        for (const requestSchema of _.entries(event.http.request.schema)) {
          ...
          if (typeof schema !== 'string' || !apiGatewayModels[schema]) {
            ...
            modelLogicalId = apiGatewayModels[schema].modelLogicalId;
            validatorLogicalId = apiGatewayModels[schema].validatorLogicalId;
          }
  ...
  // Can probably be extended as needed, but keeping this generic
  unstructuredContentType(contentType) {
    return contentType.includes('text/html') || contentType.include('text/plain');
  },
};
