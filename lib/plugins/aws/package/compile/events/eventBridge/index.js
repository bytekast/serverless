          ...
          if (event.eventBridge) {
            ...
            if (!useNativeCloudFormation) {
              this.serverless._logDeprecation(
                'EVENT_BRIDGE_CUSTOM_RESOURCE',
                'Prior to version XXXX EventBridge resources were not created using native CloudFormation, this is now possible and can be turned of by setting `useNativeCloudFormation` in the EventBridge event definition. In order to use this native CloudFormation you should destroy your stack and recreate, alternatively you can perform a manual import of existing reources and see how that goes. In the next majoor release deployment using custom resources will be deprecated and all EventBridge events will be deployed using native CloudFormation'
              );
