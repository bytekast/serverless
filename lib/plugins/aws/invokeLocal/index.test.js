...
describe.skip('test/unit/lib/plugins/aws/invokeLocal/index.test.js', () => {
  const testRuntime = (functionName = 'hello', cliParams = []) => {
    describe('Input resolution', () => {

      it('should accept no data', async () => {
        const testResult = await runServerless({ 
          fixture: 'functionInvocation', 
          cliArgs: ['invoke', 'local', '--function', functionName, 
          ...cliParams], 
        })

        expect(testResult).to.be.fulfilled.then(({stdoutData}) => {
          expect(stdoutData).to.not.be.empty;
        })
      })
      ...
      it('should support plain string data', async () => {
        ...
        expect(testResult).to.be.fulfilled.then(({stdoutData}) => {
          expect(stdoutData).to.include('inputData')
        })
      })
    })
    ...
    describe('Automated JSON parsing', () => {
      let testResult
      before(async () => {
        testResult = await runServerless({
          fixture: 'functionInvocation',
            cliArgs: [
              'invoke',
              'local',
      ...
      it('should support JSON string data', () => {
        expect(testResult).to.be.fulfilled.then(({stdoutData}) => {
          expect(stdoutData).to.deep.equal({ inputKey: 'inputValue' });
        })
      })

      it('should support JSON string client context', () => {
        expect(testResult).to.be.fulfilled.then(({stdoutData}) => {
          expect(stdoutData).to.deep.equal({ key: 'value' });
        })
      })
    })
    ...
    describe('--raw option', () => {
      ...
      it('should not attempt to parse data with the raw option', () => {
        expect(testResult).to.be.fulfilled.then(({stdoutData}) => {
          expect(stdoutData).to.deep.equal('{"inputKey":"inputValue"}');
        })
      })
      ...
      before(async () => {
        testResult = await runServerless({
          ...
          cliArgs: [
            ...
            ...cliParams,
            '--path',
            'test/fixtures/functionInvocation/payload.json',
            '--contextPath',
            'test/fixtures/functionInvocation/context.json'
            ],
        })
      })

      it('should support JSON file path as data', () => {
        expect(testResult).to.be.fulfilled.then(({stdoutData}) => {
          expect(stdoutData.path).to.deep.equal('test/fixtures/functionInvocation/payload.json');
        })
      })

      it('should support JSON file path as client context', () => {
        expect(testResult).to.be.fulfilled.then(({stdoutData}) => {
          expect(stdoutData.contextPath).to.deep.equal('test/fixtures/functionInvocation/context.json');
        })
      });
    })
    ...
    it('should support YAML file path as data', async () => {
      const testResult = await runServerless({
          ...
          cliArgs: [
            ...
            ...cliParams,
            '--path',
            'test/fixtures/functionInvocation/payload.yaml',
          ],
        });

        expect(testResult).to.be.fulfilled.then(({stdoutData}) => {
          expect(stdoutData.path).to.deep.equal('test/fixtures/functionInvocation/payload.yaml');
        }) 
    })
    ...
    it('should support JS file path for data', async () => {
      const testResult = await runServerless({
        ...
        expect(testResult).to.be.fulfilled.then(({stdoutData}) => {
          expect(stdoutData.path).to.deep.equal('test/fixtures/functionInvocation/payload.js');
        }) 
    })

    it('should support absolute file path as data', async () => {
      const absolutePath = await getTmpDirPath();
      const dataFile = path.join(absolutePath, 'data.json')
      const testResult = await runServerless({
        fixture: 'functionInvocation',
        cliArgs: [
          'invoke',
        ...
        expect(testResult).to.be.fulfilled.then(({stdoutData}) => {
          expect(stdoutData.path).to.deep.equal(dataFile);
        }) 
    })
    ...
    it('should throw error if data file path does not exist', async () => {
      const testResult = await runServerless({
        fixture: 'functionInvocation',
        cliArgs: ['invoke', 'local','--function', functionName, '--path', 'not-existing.yaml'],
      })

      testResult.to.eventually.be.rejected.and.have.property('statusCode: 404')
    })

    it('should throw error if function does not exist', async () => {
      const testResult = await runServerless({
        fixture: 'functionInvocation',
        cliArgs: ['invoke', 'local','--function', 'not-a-function', '--path', 'not-existing.yaml'],
      })
    ...
    it('should expose eventual AWS credentials in environment variables', () => {
      testResult.then(({stdoutData}) => {
        expect(stdoutData.key).to.be.equal('AAKIXXX')
        expect(stdoutData.secret).to.be.equal('ASAKXXX')
      })
    })

    it('should expose `provider.env` in environment variables', () => {
      testResult.then(({stdoutData}) => {
        expect(stdoutData.PROVIDER_LEVEL_VAR).to.be.equal('PROVIDER_LEVEL_VAR_VALUE')
      })
    })

    it('should expose `provider.profile` in environment variables', () => {
      testResult.then(({stdoutData}) => {
        expect(stdoutData.AWS_PROFILE).to.be.equal('production')
      })
    })

    it('should expose `functions[].env` in environment variables', () => {
      testResult.then(({stdoutData}) => {
        expect(stdoutData.FUNCTION_LEVEL_VAR).to.be.equal('FUNCTION_LEVEL_VAR_VALUE')
      })
    })

    it('should expose `--env` vars in environment variables', () => {
      testResult.then(({stdoutData}) => {
        expect(stdoutData.PARAM_ENV_VAR).to.be.equal('param-env-var-value')
      })
    })

    it('should expose default lambda environment variables', () => {
      testResult.then(({stdoutData}) => {
        expect(stdoutData.LANG).to.be.equal('en_US.UTF-8');
        expect(stdoutData.AWS_REGION).to.be.equal('us-east-2');
        expect(stdoutData.AWS_DEFAULT_REGION).to.equal('us-east-2');
        expect(stdoutData.AWS_LAMBDA_FUNCTION_MEMORY_SIZE).to.be.equal('1024');
        expect(stdoutData.NODE_PATH).to.equal('/var/runtime:/var/task:/var/runtime/node_modules');
        expect(stdoutData.AWS_LAMBDA_FUNCTION_VERSION).to.equal('$LATEST');
        expect(stdoutData.AWS_LAMBDA_FUNCTION_NAME).to.equal('fn');
      })
    })

    it('should resolve region from `service.provider` if not provided via option', () => {
      testResult.then(({serverless}) => {
        serverless.provider.options.region = null;
        serverless.service.provider.region = 'us-west-1';
        expect(serverless.stdoutData.AWS_REGION).to.equal('us-west-1');
        expect(serverless.stdoutData.AWS_DEFAULT_REGION).to.equal('us-west-1');
      })
    })
  })
