  ...
  const testRuntime = (functionName = 'hello', cliParams = []) => {
    ...
    describe('JSON file input', () => {
      ...
      it('should support JSON file path as data', () => {
        expect(testResult).to.be.fulfilled.then(({stdoutData}) => {
          expect(stdoutData.path).to.deep.equal('payload.json');
        })
      })

      it('should support JSON file path as client context', () => {
        expect(testResult).to.be.fulfilled.then(({stdoutData}) => {
          expect(stdoutData.contextPath).to.deep.equal('context.json');
        })
      });
    })
    ...
    it('should support YAML file path as data', async () => {
      const testResult = await runServerless({
        ...
        expect(testResult).to.be.fulfilled.then(({stdoutData}) => {
          expect(stdoutData.path).to.deep.equal('payload.yaml');
        }) 
    })
    ...
    it('should support JS file path for data', async () => {
      const testResult = await runServerless({
        ...
        expect(testResult).to.be.fulfilled.then(({stdoutData}) => {
          expect(stdoutData.path).to.deep.equal('payload.js');
        }) 
    })
