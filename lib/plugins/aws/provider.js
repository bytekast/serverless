...
class AwsProvider {
  constructor(serverless, options) {
    ...
    this.hooks = {
      initialize: () => {
        ...
        this.reportDeprecatedUsage('PROVIDER_IAM_SETTINGS', [
          ['role', 'iam.role'],
          ['rolePermissionsBoundary', 'iam.role.permissionBoundary'],
          ['iamManagedPolicies', 'iam.role.managedPolicies'],
          ['iamRoleStatements', 'iam.role.statements'],
          ['cfnRole', 'iam.deploymentRole'],
        ]);
      },
    };
  ...
  reportDeprecatedUsage(deprecationCode, pairs) {
    for (const [deprecated, modern] of pairs) {
      if (_.has(this.serverless.service.provider, deprecated)) {
        this.serverless._logDeprecation(
          deprecationCode,
          `Starting with version 3.0.0, "provider.${deprecated}" property will be replaced with "provider.${modern}"`
        );
      }
    }
  }
...
Object.defineProperties(
  ...
  memoizeeMethods({
    ...
    resolveImageUriAndShaFromPath: d(
      async function (imageName, imagePath, imageFilename) {
        ...
        const buildDockerArgs = ['build', '-t', localTag, '-f', pathToDockerfile, imagePath];
        try {
        ...
        if (_.isObject(imageDefinedInProvider)) {
          ...
          if (imageDefinedInProvider.path) {
            return await this.resolveImageUriAndShaFromPath(
              imageName,
              imageDefinedInProvider.path,
              imageDefinedInProvider.file || defaultDockerfile
            );
          }
