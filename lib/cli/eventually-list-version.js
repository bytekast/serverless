...
module.exports = async () => {
  ...
  if (localServerlessPath) {
    ...
    const isLocal =
      path.resolve(localServerlessPath, '../cli/eventually-list-version.js') === __filename;
    if (!isLocal) {
      ...
      try {
        try {
          return require(path.resolve(localServerlessPath, '../../bin/serverless.js'));
        } catch {
          // Pass and attempt to use `bin/serverless` that was used by older version of the Framework
          return require(path.resolve(localServerlessPath, '../../bin/serverless'));
        }
      } catch {
