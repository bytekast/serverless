...
module.exports = async (exception, options = {}) => {
  ...
  if (isInvokedByGlobalInstallation) {
    ...
    try {
      ...
      try {
        return require(path.resolve(localServerlessPath, '../cli/handle-error'))(exception, {
          isLocallyInstalled,
          isUncaughtException,
        });
      } catch (error) {
        ...
        return require(path.resolve(localServerlessPath, '../classes/Error')).logError(exception, {
